...
class Currency_rate_update_service(osv.Model):
    ...
    _columns = {
                    ...
                    'service' : fields.selection(
                                                    ...
                                                    ('BOC_getter','Bank of China rates'),
...
class Currency_rate_update(osv.Model):
    ...
    cron = {
            ...
            'interval_number' : 1,
            'interval_type'   : 'weeks',
            'nextcall'        : time.strftime("%Y-%m-%d %H:%M:%S", (datetime.today() + timedelta(days=0)).timetuple() ), #tomorrow same time
            'numbercall'      : -1,
            'doall'           : True,
    ...
    def run_currency_update(self, cr, uid):
        ...
        #_logger.info("zuoqin - start currency updater")
        factory = Currency_getter_factory()
        curr_obj = self.pool.get('res.currency')
        rate_obj = self.pool.get('res.currency.rate')
        companies = self.pool.get('res.company').search(cr, uid, [])

        #_logger.info("zuoqin - before for companies")
        for comp in self.pool.get('res.company').browse(cr, uid, companies):
            ...
            if not comp.auto_currency_up :
                #str1 = "zuoqin - this company is not automatic updater" + comp.name
                #_logger.info(str1)
                continue
            #else:
            #    str1 = "zuoqin - this company is automatic updater: " + comp.name
            #    _logger.info(str1)
            #we initialise the multi company search filter or not serach filter
            search_filter = []
            
            #str1 = "zuoqin - before search_filter"
            #_logger.info(str1)

            if comp.multi_company_currency_enable :
                search_filter = [('company_id','=',comp.id)]

            #str1 = "zuoqin - before main_curr_ids"
            #_logger.info(str1)
            ...
            if not main_curr_ids:
                ...
                #str1 = "zuoqin - before main_curr_ids no company"
                #_logger.info(str1)
            ...
            if main_curr_ids:
                ...
                #str1 = "zuoqin - main_curr_ids after main_curr_ids[0]"
                #_logger.info(str1)
            else:
                #str1 = "zuoqin - main_curr_ids error"
                #_logger.info(str1)
                raise orm.except_orm(_('Error!'),('There is no base currency set!'))


            if main_curr_rec.rate != 1:
                #str1 = "zuoqin - error!!! main_curr_rec: " + str(main_curr_rec.rate)
                #_logger.info(str1)
                raise orm.except_orm(_('Error!'),('Base currency rate should be 1.00!'))
            #else:
            #    str1 = "zuoqin - it is OK: main_curr_rec: " + str(main_curr_rec.rate)
            #    _logger.info(str1)

            main_curr = main_curr_rec.name
            for service in comp.services_to_use:
                #str1 = "Starting comp.services_to_use =" #+ comp.services_to_use
                #_logger.info(str1)
                ...
                try :
                    ...
                    #_logger.info("zuoqin - starting db write")
            ...
            #_logger.info("zuoqin - after for 1 companies")
        #_logger.info("zuoqin - after for 2 companies")
...
class Curreny_getter_interface(object) :
    ...
    supported_ch_currency_array = \
    ['USD 美元', 'EUR 欧元', 'AUD 澳大利亚元', 'JPY 日元', 'HKD 港币', 'GBP 英镑', 'NZD 新西兰元', 'MYR 林吉特', 'RUB 卢布', 'CAD 加拿大元', 'SGD 新加坡元']
...
##CN BOC #####   Bank of China   ############################################################
class BOC_getter(Curreny_getter_interface) :
    """Implementation of Curreny_getter_factory interface for Bank of China Web site"""

    def get_currency_rate_boc(self, currency, html1):
    #----------------------------------------------------------------------------------------------
        html = html1.encode('utf8')
        ind1 = 0
        #for l in re.finditer("<td height=\"22\" align=\"left\"><a href=\"#\" class=\"hei12\"><A href=\'",html):
        str1 = "<td>" + currency[1] + "</td>"
        for l in re.finditer(str1,html):
            ind2 = l.start()
            if(ind2 > ind1):
                ind1 = ind2
                break
        ...
        ind2 = 0
        for l in re.finditer("<td>",html):
            ind2 = l.start()
            if(ind2 > ind1):
                ind1 = ind2 + 4
                break
        
        #print("ind1_2 = " + str(ind1))
        ...
        rate = float(html[ind1:ind2].encode('utf8'))
        #print(rate)
        self.updated_currency[currency[0]] = rate / 100.0
        #ref1 = "http://www.pbc.gov.cn/" + html[ind1:ind2].encode('utf8')
    #----------------------------------------------------------------------------------------------

    def get_updated_currency(self, currency_array, main_currency, max_delta_days):
        #"""implementation of abstract method of Curreny_getter_interface"""

        # as of Jan 2014 BOC is publishing noon rates for 3 currencies
        #url = 'http://www.bankofcanada.ca/stats/assets/rates_rss/noon/en_%s.xml'
        url = "http://www.pbc.gov.cn/publish/zhengcehuobisi/637/index.html"
        ...
        if r.status_code == 200:
            ...
            #print("Error in retrieving html")
            #print(r.status_code)
            return 0;
            #quit()
        #f = open("/Users/zuoqin/Downloads/boc01.html", 'r')
        #html = f.read().decode('utf8')
        #f.close()

        
        a = map(lambda x: x.split(), self.supported_ch_currency_array)
        for b in a:
            #print(b[1])
            self.get_currency_rate_boc(b, html)
